...
|how many times it worked| how many times it didin't work|
|:------|:------|
|70%|    30%|

steps to get the hacks
- download tampermonkey beta or original

- copy this code below

- open tampermonkey and click on create a new script

- then paste the code and enable it

open narwhale.io and have fun! :)

```
// ==UserScript==
// @name         Narwhale.io Zoom - Narwhaleio Hacks - Best Narwhale Cheat 2019 to 2020
// @description  Narwhaleio Mods Features: Aimbot, Zoom, ESP, Auto Turn, FPS, Adblock
...
// @updateURL    https://io-mods.com/mods/narwhaleio.user.js
// @downloadURL  https://io-mods.com/mods/narwhaleio.user.js
// @grant          GM_setValue
// @grant          GM_getValue
// @match        *://narwhale.io/*
// @match        *iogames.space/*
...
text += '<a href="http://'+value+'" target="_blank" style="'+ministyler+'">'+value2+' <font color="#555555">-</font></a>';
    ...
    //ozellikler
    var fts = ["Auto Aim","Aimbot","Firebot","Speed Hack","Heal Hack","Show Players","Auto Fire","Anti Lag"];
...
//degisenkisimlar
$('#adZone').html('<div style="'+this.st.formstyle+'"><div class="option1"></div></div>');
$('#adZone').append('<div style="'+this.st.fpsstyle+'" id="fps" class="fps">');
$('#shareBox').prepend('<hr><div class="list1"></div>');
...
document.getElementById("inputLayout").appendChild(stylerSelect);
...
function changebackground() {
	...
	$("#spawnbox").css("background-color",rgbaC2);
}
...
function colorfulmod() {
    ...
    colorsrain = ["transparent"];
      ...
      setInterval(function() {
		  ...
		  $("#spawnbox").css("background-color",rgbaCol);
      }, 3000);
}
...
(function() {
    ...
    window.addEventListener("wheel", event => {
         if (event.deltaY < 0)
         {
            if (game.options.viewScale < 5) game.options.viewScale += 0.015;
            game.handleResize();
             game.dashable=true;
        game.isDead=false;
        } else if (event.deltaY > 0) {
            if (game.options.viewScale > 0.1) game.options.viewScale -= 0.01;
            game.handleResize();
                    game.dashable=true;
        game.isDead=false;
        }
